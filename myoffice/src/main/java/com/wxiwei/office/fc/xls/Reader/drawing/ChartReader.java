...
import com.wxiwei.office.thirdpart.achartengine.chart.ColumnBarChart.Type;
    ...
    private int getColor(Element solidFillElement)
    {
        ...
        else if(solidFillElement.element("schemeClr") != null)
        {
            ...
            if(clr.element("lumMod") != null)
            {
                double tint = 0;
                try {
                    tint = Integer.parseInt(clr.element("lumMod").attributeValue("val")) / 100000.0;
                } catch (NumberFormatException e) {
                    e.printStackTrace();
                }
...
//            type = AbstractChart.CHART_BUBBLE;
    ...
    private void setSeriesRendererProp(XYMultipleSeriesRenderer renderer, Element series, PointStyle[] styles)
    {
        ...
        int seriesOrder = 0;
        try {
            seriesOrder = Integer.parseInt(series.element("order").attributeValue("val"));
        } catch (NumberFormatException e) {
            e.printStackTrace();
        }
