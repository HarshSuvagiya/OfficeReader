...
public class SheetReader
{
    ...
    private void setColumnProperty(Element col)
    {
        int min = 0;
        try {
            min = Integer.parseInt(col.attributeValue("min")) - 1;
        } catch (NumberFormatException e) {
            e.printStackTrace();
        }
        int max = 0;
        try {
            max = Integer.parseInt(col.attributeValue("max")) - 1;
        } catch (NumberFormatException e) {
            e.printStackTrace();
        }
        int styleIndex = 0;
    ...
    class XLSXSaxHandler implements ElementHandler
    {
        ...
        public void onEnd(ElementPath elementPath)
        {
            ...
            else if (name.equals("row"))
            {
                int rowIndex = 0;
                try {
                    rowIndex = Integer.parseInt(elem.attributeValue("r")) - 1;
                } catch (NumberFormatException e) {
                    e.printStackTrace();
                }
    ...
    private  Row createRow(Element rowElement, int defaultRowHeight)
    {
        ...
        int rowIndex = 0;
        try {
            rowIndex = Integer.parseInt(rowElement.attributeValue("r")) - 1;
        } catch (NumberFormatException e) {
            e.printStackTrace();
        }
