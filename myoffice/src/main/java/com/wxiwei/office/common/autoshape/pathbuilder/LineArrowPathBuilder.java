...
//            }
        ...
        float endX, float endY, float fU)
   {
    ...
    public static PointF getReferencedPosition(Element head, PointF tail, byte arrowType)
    {
        float x = 0;
        try {
            x = Integer.parseInt(head.attributeValue("x")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
        } catch (NumberFormatException e) {
            e.printStackTrace();
        }
        float y = 0;
        try {
            y = Integer.parseInt(head.attributeValue("y"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
        } catch (NumberFormatException e) {
            e.printStackTrace();
        }
