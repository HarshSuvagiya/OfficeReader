        ...
        Element sp, BackgroundAndFill fill, boolean border, BackgroundAndFill lineFill, Element ln, Rectangle rect) throws Exception
    {
        ...
        if (pathElements.size() == 1 && ln != null)
        {
             ...
             if (width != null && height != null)
             {
                 float w = 0;
                 try {
                     w = Integer.parseInt(width) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
                 } catch (NumberFormatException e) {
                     e.printStackTrace();
                 }
                 float h = 0;
                 try {
                     h = Integer.parseInt(height) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
                 } catch (NumberFormatException e) {
                     e.printStackTrace();
                 }
                    ...
                    if(arrowPathAndTail != null)
                    {
                        ...
                        if(path1 != null)
                        {
                            ...
                            if (fill != null || border)
                            {
                                int kju= 0;
                                try {
                                    kju = Integer.parseInt(pathElement1.attributeValue("stroke"));
                                } catch (NumberFormatException e) {
                                    e.printStackTrace();
                                }
                                if (border && 
        ...
        for (int j = 0; j < pathElements.size(); j++)
        {
            ...
            if (fill != null || border)
            {
                ...
                if (border)
                {
                    int kju= 0;
                    try {
                        kju = Integer.parseInt(pathElement.attributeValue("stroke"));
                    } catch (NumberFormatException e) {
                        e.printStackTrace();
                    }
