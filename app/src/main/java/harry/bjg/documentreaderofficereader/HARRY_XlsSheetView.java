...
public class HARRY_XlsSheetView extends TextureView {
    ...
    public HARRY_XlsSheetView(Context context) {
        super(context);
    }

    public HARRY_XlsSheetView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public HARRY_XlsSheetView(Context context, AttributeSet attrs, int defStyleAttr) {
                ...
                if (!StringUtils.isEmpty(cell.getContents()) && !content.contains("\n")) { //non empty single line
                    ...
                    if (size > columnSizes[i + 1] - columnSizes[i]) {
                        ...
                        while (finalColumn + 1 < sheet.getColumns() &&
                                ...
                                StringUtils.isEmpty(sheet.getCell(finalColumn + 1, j).getContents()))
                            finalColumn++;
                        if (finalColumn != i) {
    ...
    private boolean drawSheet(Canvas canvas) {
        ...
        paint.setColor(0xffcccccc);
        if (fixedColumns > 0)
            canvas.drawRect(fixedColumnSize + l - dp, t, fixedColumnSize + fixColumnOffset + l, b, paint);
        if (fixedRows > 0)
            canvas.drawRect(l, fixedRowSize + t - dp, r, fixedRowSize + fixRowOffset + t, paint);
    ...
    private void drawBorder(float x, float y, float w, float h, BorderLineStyle style, Colour color, Canvas canvas) {
        ...
        if (color.getValue() == 64)
            paint.setColor(Color.BLACK); //set automatic color tmpRectF black, default is white
        else {
    ...
    private void drawText(String id, int x, int y, int w, int h, String text, boolean isMerged, CellFormat cellFormat, Canvas canvas) {
        ...
        if (canvas.clipRect(0, 0, fw, fh)) {
            ...
            if (cellFormat != null) {
                if (cellFormat.getAlignment() == Alignment.RIGHT || (cellFormat.getAlignment() == Alignment.GENERAL && isRightAlignDefault(text, cellFormat)))
                    hAlign = Layout.Alignment.ALIGN_OPPOSITE;
            ...
            if (layoutsMap.containsKey(id)) layout = layoutsMap.get(id);
                ...
                if (useBoringLayout) {
                    ...
                    if (metrics != null)
                        layout = BoringLayout.make(text, textPaint, fw, hAlign, 1, 0, metrics, false);
                }
                if (layout == null)
