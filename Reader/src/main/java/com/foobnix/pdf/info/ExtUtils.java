...
public class ExtUtils {
    ...
    public static boolean showDocumentWithoutDialog2(final Context c, final File file) {
        ...
        Help1.width = c.getResources().getDisplayMetrics().widthPixels;
        Help1.height = c.getResources().getDisplayMetrics().heightPixels;

        final Dialog dialog = new Dialog(c);
        dialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        dialog.setContentView(R.layout.dialog_selectmode);

        ImageView btnscroll = dialog.findViewById(R.id.btnscroll);
        ImageView btnbook = dialog.findViewById(R.id.btnbook);
        LinearLayout laymain = dialog.findViewById(R.id.laymain);
        TextView title = dialog.findViewById(R.id.title);

        title.setTypeface(Typeface.createFromAsset(c.getAssets(), "arial.ttf"));
        Help1.setSize(laymain, 978, 750, false);
        Help1.setSize(btnscroll, 352, 396, false);
        Help1.setSize(btnbook, 352, 396, false);
        Help1.setMargin(btnbook, 70, 0, 0, 0, false);
